...
void main() {
	...
	// wave-like noise displacement
	vec2 nPos = ( dim * 0.5 + pos.xz ) / dim;
	pos.y = cnoise( vec3( nPos * vec2( 7.6, 3.8 ), time ) ) * 8.0;
	pos += rand;
	...
	// h = < p-a, b-a > / | b-a |^2
	// q = a + h * ( b - a )
	vec3 pa = cur - mousePrev, ba = mousePosition - mousePrev;
	vec3 q = mousePrev + ba * clamp( dot( pa, ba ) / dot( ba, ba ), 0.0, 1.0 );

	vec3 offset = cur - q;
	float nDist = length( offset ) / mouseRadius;

	// mouse perturbation displacement
	if ( nDist < 1.0 ) vel += normalize( offset ) * mix( 7.0, 0.1, nDist ) + rand * 0.02;
