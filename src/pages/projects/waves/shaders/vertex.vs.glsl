...
void main() {
  ...
  isTopFace = dot(vec3(0., 1., 0.),vNormal);
  vPosition = position;
  scale = vec2(10.);

  elevation = fbmWaves(position.xz);
  vPosition.y += elevation * smoothstep(.3, 1., vPosition.y + 1.);

  modelPosition = modelMatrix * vec4(vPosition, 1.0);
